...
export function add(content) {
    ...
    return dispatch => {
        fetch('/api/todo', {method: 'POST', body: JSON.stringify({content})})
            .then(res=>res.json()).then(json => dispatch({type:ADD_TYPE, id:json.id}));
    }
}
...
export function done(id) {
    ...
    return dispatch => {
        fetch(`/api/todo/${id}/done`, {method:'PUT'})
            .then(res=>res.json()).then(json => dispatch({type: DONE_TYPE, id:json.id}));
    }
}
...
export function reopen(id) {
    ...
    return dispatch => {
        fetch(`/api/todo/${id}/reopen`, {method:'PUT'})
            .then(res=>res.json()).then(json => dispatch({type: REOPEN_TYPE, id:json.id}));
    }
}
...
export function list(fl) {
    return dispatch  => {
        fetch(`/api/todo/${fl}`).then(res => res.json()).then(json => dispatch({ type:LIST_TYPE, todos: json.todo}))
    };
};